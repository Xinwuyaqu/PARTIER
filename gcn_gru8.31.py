...
test_datas = ['PARSEC', 'hpcc', 'hpl-s', 'hpcg', 'SPEC', 'GRAPH500', 'HPL-AI', 'LmBench', 'MiBench', 'RoyBench', 'SMG2000']
...
def create_dataset(dataset, label, look_back=time_step, datatype=0):
    ...
    dataX, dataY = [], []
    test_scale = data_scale
    if datatype != 0:
        test_scale = data_scale // 2
    for p in range(0, test_scale):
        i = random.randint(0, len(dataset) - look_back - 12)
        if datatype == 0:
            i = (i//20)*20+i%10
        else:
            i = (i//20)*20+i%10+10
