...
def get_gcn_model(data_shape):  # 静态。edge_layer是描述边的矩阵（N*N），data_layer是描述点的矩阵(N*feature_dim)
    ...
    def merge(input):
        tem = [input[i] for i in range(data_shape)]
        return K.stack(tem, 1)
    def avg_pool(input):
        data = K.sum(input, axis=-2)
        data = data * 0.1
        return data
    def max_pool(input):
        data = K.max(input, axis=-2)
        return data
    def min_pool(input):
        data = K.min(input, axis=-2)
        return data
    ...
    input0 = Lambda(slice, name="slice_feature0", output_shape=(1,), arguments={'index': 0})(input_layer)
    ...
    embed0 = Embedding(input_dim=interval_length, output_dim=DATA_DIM, input_length=1, trainable=True)(input0)
    ...
    embed = Lambda(merge, name="merge", output_shape=(data_shape, DATA_DIM,))(
        [embed0, embed1, embed2, embed3, embed4, embed5, embed6, embed7, embed8, embed9])

    edge_layer = Input(shape=(len(dense_features), len(dense_features)))
    conv_layer = GraphConv(
        units=DATA_DIM,
        step_num=1,
    )([embed, edge_layer])

    tem_target = Lambda(avg_pool, name="avg_pool", output_shape=(DATA_DIM,))(conv_layer)
    #tem_target = Lambda(min_pool, name="min_pool", output_shape=(DATA_DIM,))(conv_layer)
    #tem_target = Lambda(max_pool, name="max_pool", output_shape=(DATA_DIM,))(conv_layer)

    model = Model([input_layer, edge_layer], tem_target, name="static_model")
...
def get_gru_model(data_shape):
    ...
    input1 = Input(shape=(data_shape,), name="time_t1_input")
    input2 = Input(shape=(data_shape,), name="time_t2_input")
    input3 = Input(shape=(data_shape,), name="time_t3_input")
    edge_layer = Input(shape=(len(dense_features), len(dense_features)), name="edge_layer")
...
def get_edge_matrix():
    ...
    try:
        ...
        pass
    print([col.index(each) for each in dense_features])
    edge_matrix = np.array(tem[dense_features])[[col.index(each) for each in dense_features]]
    print(edge_matrix)
...
def get_data(paths):
    ...
    for path in paths:
        ...
        data = mapPMCs(data)
        data, label = create_dataset(data[dense_features], np.array(data[targets]), time_step)
        print(len(data))
