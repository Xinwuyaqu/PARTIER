...
field1 = (['SPEC','hpcc','hpcg'],['hpl'])    # High performance Computing
field2 = (['SPEC','vips','x264','facesim'],['fluidanimate'])          # Multimedia
field3 = (['hpl-ai-medium','hpl-ai-large'],['hpl-ai-small'])       # Artificial Intelligence

train_datas,test_datas=field3
...
#构造GRU的输入
def create_dataset(dataset, label, look_back=time_step, datatype=0):
...
def get_gcn_model(data_shape):  # 静态。edge_layer是描述边的矩阵（N*N），data_layer是描述点的矩阵(N*feature_dim)
    def extract(input):
        return input[:, 5, :]  #返回input[:,0,:]为 target 功耗
...
def get_data(paths, datatype=0):
    ...
    for path in paths:
        ...
        data = mapPMCs(data)
        special_node = ["target"]
        data, label = create_dataset(data[special_node + dense_features], np.array(data[targets]), time_step, datatype)
        print(len(data))
...
def train():
    train_data, train_label = get_data(train_paths, datatype=0)#加载训练集
    test_data, test_label = get_data(test_paths, datatype=1)#加载测试集
